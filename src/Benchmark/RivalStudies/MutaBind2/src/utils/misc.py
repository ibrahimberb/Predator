...
def create_mutation_file(path, filename, mutation, chain_to_mutate):
    """
    # #Chain to Mutate	Residue	Mutant Residue
    # #split by tab
    # E	A18	G
    """
    residue, mutant_residue = mutation[:-1], mutation[-1]
    mutation_filepath = op.join(path, filename)
    with open(mutation_filepath, 'w') as f:
        f.write(chain_to_mutate + '\t' + residue + '\t' + mutant_residue)

    log.debug(f"Temporary file {filename} is created.")
